...
CPUE <- bind_rows(fsu, celr, logbook) %>%
  mutate(logSD = log(1 + SD / Mean), Season = ifelse(Season == "AW", 1, 2)) %>%
  left_join(ys, by = join_by(Year, Season)) %>%
  select(Year, Season, Period, Median, logSD, q)
tail(CPUE)

l_cpue <- c("AW FSU", "SS FSU", "AW CELR", "SS CELR", "AW Logbook", "SS Logbook")
...
fun <- function(parameters, data) {
  ...
  biomass[1] <- K
  for (t in 2:n_time) {
    biomass[t] <- B[t - 1] + production[t - 1] - catch_obs[t - 1]
  }
...
cpue_df <- data.frame(Year = CPUE$Period, Observed = CPUE$Median, Predicted = obj$report()$cpue_pred, Index = l_cpue[CPUE$q]) %>% 
...
pred_Bbdm <- data.frame(Year = catch$Year, Season = catch$Season, B = obj$report()$biomass, Model = "BDM")
